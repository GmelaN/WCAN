...
int main(int argc, char *argv[]) {
    ...
    NodeContainer reducedFunctionalDevices;
    reducedFunctionalDevices.Create(10);
    ...
    mobilityHelper.Install(reducedFunctionalDevices);

    // 1-2. Coordinator 설정
    Ptr<ListPositionAllocator> listPositionAllocator = CreateObject<ListPositionAllocator>();
    listPositionAllocator->Add(Vector(25, 25, 0));
    mobilityHelper.SetPositionAllocator(listPositionAllocator);

    // 2. NetDevice 설정
    LrWpanHelper lrWpanHelper;
    NetDeviceContainer netDevices = lrWpanHelper.Install(reducedFunctionalDevices);

    netDevices.Add(lrWpanHelper.Install(coordinator)); // ??????


    // 3. callback 설정
    // 3-1. RFD 설정
    for(auto i = reducedFunctionalDevices.Begin(); i != reducedFunctionalDevices.End(); i++) {
        Ptr<Node> node = *i;
        ...
        // RFD 개별 스캔 시작
        Time jitter = Seconds(2) + MilliSeconds(std::distance(reducedFunctionalDevices.Begin(), i) * 1000);
    ...
    // 3-2. 코디네이터 설정
    Ptr<LrWpanNetDevice> coordinatorNetDevice = DynamicCast<LrWpanNetDevice>(coordinator->GetDevice(0));
    ...
    coordinatorNetDevice->GetMac()->SetShortAddress(Mac16Address("FF:FE")); // 0xFFFF보다 작은 값으로 설정해야 함


    // 5. 코디네이터(PAN ID: 5) 신호 전송 시작
    ...
    Simulator::ScheduleWithContext(coordinatorNetDevice->GetNode()->GetId(),
                                   Seconds(0.0),
    ...
    // 6. 한 RFD에서 코디네이터로 데이터 전송
    std::string string = "Hi there!";
    Ptr<Packet> packet = Create<Packet>((uint8_t*) string.c_str(), string.length());

    // 한 RFD의 NetDeivce
    Ptr<LrWpanNetDevice> someRfdNetDevice = DynamicCast<LrWpanNetDevice>(reducedFunctionalDevices.Get(0)->GetDevice(0));
    ...
    // 코디네이터에 데이터 수신 완료 시 패킷 내용을 출력하는 콜백 함수 설정
